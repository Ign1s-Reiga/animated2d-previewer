...
impl GpuContext {
    ...
    pub fn headless() -> Result<GpuContext, RenderError> {
        GpuContext::request(None)
    }

    /// Acquires a device compatible with `surface`.
    pub fn for_surface(surface: &wgpu::Surface<'_>) -> Result<GpuContext, RenderError> {
        GpuContext::request(Some(surface))
    }

    /// Wraps a device the caller already owns.
    pub fn from_parts(device: Arc<wgpu::Device>, queue: Arc<wgpu::Queue>) -> GpuContext {
        GpuContext { device, queue, adapter_name: "provided by host".into(), backend: "unknown".into() }
    }

    fn request(compatible_surface: Option<&wgpu::Surface<'_>>) -> Result<GpuContext, RenderError> {
