...
> Cubism モデルは MOC3 を独自実装で読み、ポーズ・クリッピング・描画順まで含めて
> ビューアに表示できます。モーション（Unity AnimationClip）は未対応です。
...
| `a2d-cubism` | **一部実装** — MOC3 読み取り（ID / カウント / キャンバス / パラメータ範囲 / ドローアブルの UV・三角形 / キーフォーム / デフォーマ / 不透明度 / クリッピングマスク / ブレンドモード / パート / 描画順）。独自パーサ方針（Cubism Core は使いません）。ポーズ・クリッピング・描画順まで正しく描画でき、`AnimatedModel` としてビューアに表示できます。未対応：Unity AnimationClip からのモーション復元、アニメーションする描画順、物理・ポーズ・表情。詳細は [`docs/cubism-moc3.md`](docs/cubism-moc3.md) |
...
本プロジェクトは、**学術的用途および個人的用途**を想定して提供されます。具体的には
次の 2 つです。

- **学術的用途** — 非公開バイナリフォーマットの解析手法、リバースエンジニアリングの
  記録、2D キャラクタランタイムの実装に関する研究・学習。この目的のために、
  各フォーマットの読み方だけでなく「何を根拠にそう判断したか」を
  [`docs/cubism-moc3.md`](docs/cubism-moc3.md) などへ併記しています。
- **個人的用途** — 利用者自身が所有するソフトウェアから、利用者自身が抽出した
  アセットを、個人的にオフラインで閲覧すること。

DRM の回避、ライセンスチェックの迂回、ゲームサーバとの通信、アカウント操作の
自動化は実装しません。ゲームアセットおよび proprietary SDK バイナリの再配布も
行いません。抽出したアセットの第三者への配布も想定していません。

アセットの取得・保持・利用が各権利者の利用規約および適用法に適合するかどうかの
判断と責任は、利用者に帰属します。

なお、ここに書いたのは**本ツールの想定用途**であり、ソースコードのライセンス条件では
ありません。コードそのものは上記のとおり MIT / Apache-2.0 のデュアルライセンスで、
その許諾範囲は本節によって狭められるものではありません。
